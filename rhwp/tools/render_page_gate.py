...
사용: python tools/render_page_gate.py --root /path/to/hwpdocs [--exe ./target/release/rhwp.exe]
      [--save out.tsv]
"""
import csv, sys, subprocess, re, argparse
from collections import Counter
from pathlib import Path

ap = argparse.ArgumentParser()
ap.add_argument("--root", required=True, help="Document corpus directory")
ap.add_argument("--exe", default=str(Path(__file__).resolve().parents[1] / "target" / "release" / ("rhwp.exe" if sys.platform == "win32" else "rhwp")))
ap.add_argument("--fixture", default=str(Path(__file__).resolve().parents[1] / "tests/fixtures/render_page_controlset.tsv"))
...
for r in rows:
    ...
    p = str(Path(a.root).joinpath(*rel.split("/")))
    try:
...
print(f"일치(rhwp==한글): {match} ({(100.0*match/total if total else 0):.1f}%)")
