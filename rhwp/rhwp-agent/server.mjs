...
const sessions = new HubSessionRegistry({
  createRecord(sessionId) {
    ...
    return {
      ...
      browserbase: new BrowserbaseFleet({ log }),
...
function makeBackendEventHandler(record, generation) {
  return (evt) => {
    ...
    if (evt.type === 'turn-end') {
      ...
      void record.browserbase.cleanupExtras('turn ended');
    }
...
function disposeSession(record) {
  ...
  void record.browserbase.cleanup('session disposed');
  if (wasRunning) {
...
async function applyBrowserbaseOverride(record, msg) {
  ...
  return record.browserbase.setOverride(override, { restart: record.agentSession?.status !== 'running' });
}
...
async function setChatWorkflow(record, sock, msg) {
  ...
  if (msg.workflow === 'direct') void record.browserbase.cleanup('workflow changed to direct');
  if (previousPlanId) {
    sendJson(sock, {
      v: 1,
      type: 'plan-invalidated',
...
async function handleStudioMessage(record, sock, msg) {
  switch (msg.type) {
    ...
    case 'browserbase-status-request': {
      const requestId = typeof msg.requestId === 'string' ? msg.requestId : null;
      replyToStudio(record, sock, { v: 1, type: 'browserbase-status', requestId, status: record.browserbase.status() });
      return;
    }
    ...
    case 'browserbase-credentials-clear': {
      const requestId = typeof msg.requestId === 'string' ? msg.requestId : null;
      void record.browserbase.setOverride(null, { restart: record.agentSession?.status !== 'running' })
        .then((status) => replyToStudio(record, sock, { v: 1, type: 'browserbase-status', requestId, status }))
        .catch((e) => sendBrowserbaseError(record, sock, requestId, e));
      return;
    }
  ...
  switch (msg.type) {
    case 'tool-call': {
      ...
      if (definition.category === 'browser') {
        ...
        void record.browserbase.call(record.agentSession.chatId, browserId, sidecarTool, sidecarArgs)
          .then(sendResult)
...
function recordHealth(record) {
  return {
    ...
    browserbase: record.browserbase.status(),
  };
}
...
async function disposeRecord(record, reason) {
  ...
  await Promise.all([
    ...
    record.browserbase.cleanup(reason),
  ]);
