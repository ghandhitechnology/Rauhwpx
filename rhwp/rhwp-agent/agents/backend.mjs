...
function editLifecycleFor(profile) {
  if (profile === 'safe') {
    return {
      ...
      tableBullet: `- For text inside a table, use find_text and pass its cell address to staged text tools; nested matches also need cellPath. If a cell edit fails, re-read its address. Do not delete or recreate the table to change its text. Table structure edits (rows, columns, merge, split) apply immediately and renumber cellIdx after the change — address later cells from the counts they return or a fresh get_structure. If a tool result carries editReport, or a turn opens with a staged_edit_report block, staged edits were discarded or stayed in the document after a rejection — re-read those parts before editing them again.`,
    };
  }
  return {
    ...
    tableBullet: `- Table structure edits (rows, columns, merge, split) apply immediately and renumber cellIdx after the change — address later cells from the counts they return or a fresh get_structure. If a tool result carries editReport, or a turn opens with a staged_edit_report block, staged edits were discarded or stayed in the document after a rejection — re-read those parts before editing them again.`,
  };
}
...
export function implementationSystemBrief(profile = 'unrestricted', agentName = 'claude') {
  ...
  const tableBullet = '- Table structure edits apply immediately and renumber cellIdx; address later cells from the counts they return or a fresh get_structure. If a tool result carries editReport, or a turn opens with a staged_edit_report block, staged edits were discarded or stayed in the document after a rejection — re-read those parts before editing them again.';
