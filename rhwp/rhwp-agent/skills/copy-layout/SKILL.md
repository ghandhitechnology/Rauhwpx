...
When removing text, the helper retains zero-width layout anchors rather than width-bearing blank strings. This keeps formerly populated floating-table hosts distinguishable from intrinsically empty anchors while preventing blank replacement text from wrapping inside dense cells and increasing row or page height.
