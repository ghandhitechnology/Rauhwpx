...
2. Report `inspecting` and run `python3` or `python` with the job's exact helper copy (`copy_layout.py` in the job workspace from the worker prompt) and `--inspect-text <snapshot>`. Never run another script, `python3 -c`/`-m`, `--rhwp-bin`, or a different interpreter. Inspect the complete paragraph inventory, fields, form controls, named structures, visual marks, and every media use—not a sample. The helper uses only Python 3's standard library; do not install `lxml`. The bundled source remains `scripts/copy_layout.py`.
