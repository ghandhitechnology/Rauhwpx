...
2. Call `get_structure` for the current revision and insertion address. Use `insert_image` with `imagePath`, `expectedRevision`, `sectionIdx`, `paraIdx`, and `charOffset`. Set a useful `description` and an explicit width when the surrounding layout calls for one.
3. Call `verify_changes` with `includeImage: true`. Check the image's placement and size, then correct any problem before finishing: `edit_object` resizes, floats and positions it in mm, or changes its wrap, without inserting it again.
