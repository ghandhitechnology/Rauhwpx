...
- Table cells: to target text inside a cell, pass cell {paraIdx, controlIdx, cellIdx}. paraIdx/controlIdx come from the table entry in get_structure tables[] (its per-cell entries do not carry them); cellIdx is the row-major index where merged cells count once. A find_text match carries a complete cell you can copy as-is. For a nested cell, also pass cellPath from find_text or get_selection (its last cellParaIndex is replaced by the tool's paraIdx). With cell, paraIdx/startParaIdx/endParaIdx and offsets are relative to the innermost cell.
