...
export const WRITE_NOTE = 'Requires expectedRevision (the revision returned by your most recent tool call). Fails with REVISION_MISMATCH if the document changed — then re-read (get_structure / get_text_range) and retry with fresh coordinates. Core text writes from sibling agents editing disjoint paragraph ranges are rebased automatically (the response then carries rebasedParaShift), so a mismatch signals a real conflict. Successful edits are staged as live preview; at turn end they are auto-committed (전체 접근 profile) or held for the user’s review and approval (안전 profile), and committed edits remain undoable in the editor.';
