...
export function cloudDocumentOwner(
  ...
  const activeSession = snapshot.session.kind === 'idle' ? null : snapshot.session;
  ...
  const leasedSessionId = snapshot.lease.owner === 'cloud' ? snapshot.lease.sessionId : null;
  if (leasedSessionId) {
    const leased = sessions.find((session) => session.sessionId === leasedSessionId);
