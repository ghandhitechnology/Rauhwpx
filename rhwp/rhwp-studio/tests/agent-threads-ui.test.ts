...
test('past chats on the active file reopen as writable and adopt stable document identity', () => {
  ...
  assert.match(source, /persistCurrentThread\(\);\s*localThreadId = currentThread\.id;[\s\S]*editorCloudScope\.bind\([\s\S]*const scopeRefresh = cloudUi\.refreshLeaseScope\(\);\s*exitReadOnlyMode\(\);[\s\S]*if \(liveQuestion\)[\s\S]*void scopeRefresh\.then/);
...
test('changing files ends the open chat and starts a fresh chat for the next file', () => {
  ...
  assert.match(documentSwitchSource, /if \(workspace\.mode\(\) === 'cloud'\)[\s\S]*localThreadSnapshot = structuredClone\(nextThread\);[\s\S]*editorCloudScope\.bind\([\s\S]*cloudUi\.refreshLeaseScope\(\)/);
