...
test('takeover completion receives the exact cached payload', async () => {
  const payload = { operationId: 'operation-a', checkpoint: 'frozen-a' };
  let completedWith: unknown = null;
  await runTakeoverAuthorityTransition({
    acquire: () => ({ release() {} }),
    prepare: async () => true,
    request: async () => payload,
    apply: async () => 'document-a',
    complete: async (...args: unknown[]) => { [completedWith] = args; },
    refresh: async () => {},
    settle: async () => {},
    onPendingChange: () => {},
    context: () => ({ profileEpoch: 1, serverIdentity: 'server-a' }),
  });
  assert.equal(completedWith, payload);
});
