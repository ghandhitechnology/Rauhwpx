...
/// 디코딩 이미지 크기에 대응시킨다. 없을 때 crop 우하단이 이미지 종횡비와
/// 일치하면 비-96dpi 스캔 그림의 전체 좌표 범위로 해석한다(#3239).
/// 종횡비가 다르면 우측/하단을 잘라낸 범위일 수 있으므로 표준 75 HU/px를
/// 사용한다. 두 축을 독립적으로 전체 이미지에 맞추면 crop이 사라진다.
            ...
            (cr > 0 && cb > 0 && img_w_px > 0.0 && img_h_px > 0.0)
                ...
                .filter(|(sx, sy)| {
                    sx.is_finite()
                        ...
                        && *sx > 0.0
                        && *sy > 0.0
                        // 이미지 픽셀 수를 정수로 저장할 때의 반올림 오차만 허용한다.
                        && ((cr as f64 / sy - img_w_px).abs() <= 1.0
                            || (cb as f64 / sx - img_h_px).abs() <= 1.0)
                })
        })
