...
const otherFrame = { ...owner, frameId: 1 };
let id = 0;
const createId = () => `transfer-${String(++id).padStart(4, '0')}`;

// Firefox structured-clones exact bounded ArrayBuffers, never the source buffer.
...
const store = new FetchTransferStore({ createId, encodeChunk: encodeArrayBufferChunk });
...
for (let index = 0; index < metadata.chunkCount; index += 1) {
  ...
  assert.ok(chunk.data instanceof ArrayBuffer);
  assert.ok(chunk.data.byteLength <= FETCH_TRANSFER_CHUNK_BYTES);
  assert.notEqual(chunk.data, source.buffer);
  rebuilt.set(new Uint8Array(chunk.data), chunk.offset);
}
assert.deepEqual(rebuilt, source);
store.close(reservation.transferId, owner);
assert.equal(store.activeCount, 0);

// Sender scope includes frame/document identity, not merely extension origin.
const isolated = store.reserve(owner);
store.commit(isolated.transferId, owner, new Uint8Array([1, 2, 3, 4]));
assert.throws(
  () => store.readChunk(isolated.transferId, 0, otherFrame),
  (error) => error instanceof FetchTransferError && error.code === 'sender-mismatch',
);
assert.throws(
  () => store.close(isolated.transferId, otherFrame),
  (error) => error instanceof FetchTransferError && error.code === 'sender-mismatch',
);
...
console.log('Firefox bounded fetch-transfer tests passed');
