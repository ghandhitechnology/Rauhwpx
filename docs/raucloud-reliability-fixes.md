...
- End arriving while Studio starts or the first turn registers resolves through the durable finish gate without dispatching a provider or counting an unfinished turn. Plan approval refreshes the advertised tool profile so implementation receives mutation tools. The authenticated Cloud worker accepts native user edits during agent turns while retaining busy-document and stale-revision guards.
...
- Reconnection retains the conversation identity and offers recovery controls after an outage. Remote input pauses while disconnected, and restarting or stopping Cloud cancels pending connection work. Rebuilding prepares the verified archived Cloud document and required references before stopping the server, retains the original publication baseline, and preserves the unsent message. A durable recovery receipt and transfer ID survive closing the app and prevent duplicate uploads on retry. Missing or corrupt recovery data stops rebuilding before the old server is removed. Cloud settings remain accessible for cleaning up a failed idle sandbox.
