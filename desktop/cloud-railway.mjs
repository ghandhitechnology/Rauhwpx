...
query RauhwpxDeploymentLogs($deploymentId: String!) {
  deploymentLogs(deploymentId: $deploymentId, limit: 30) { message }
}`;
...
export function createRailwayServerProvider({
  ...
  async function deploymentFailureDetail(deployment) {
    ...
    try {
      ...
      const lines = (Array.isArray(data?.deploymentLogs) ? data.deploymentLogs : [])
        .map((entry) => trimmed(entry?.message, 240))
        .filter(Boolean);
