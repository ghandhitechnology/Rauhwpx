...
export function createRaucloudBrokerClient({
  ...
  if (typeof authorizeOwnedBackend !== 'function') {
    throw new Error('Raucloud broker requires the account-session backend interface');
  }
  ...
  async function request(pathname, {
    ...
    try {
      ...
      const target = new URL(endpoint(normalizedBaseUrl, pathname, query));
      return await authorizeOwnedBackend({
      ...
      if (['ACCOUNT_SESSION_INVALID', 'ACCOUNT_SESSION_UNAUTHORIZED'].includes(error?.code)) {
        throw new AppServerError('Sign in to Rauhwpx to use Raucloud', {
          code: 'RAUCLOUD_AUTH_REQUIRED', retryable: false, cause: error,
        });
      }
      if (error?.code === 'RAU_CREDITS_TIMEOUT') {
        throw new AppServerError('Raucloud took too long to respond', {
          code: 'RAUCLOUD_TIMEOUT', retryable: true, cause: error,
        });
      }
      if (typeof error?.code === 'string'
        && (error.code.startsWith('RAUCLOUD_') || error.code.startsWith('CLOUD_')
          || error.code.startsWith(LEGACY_RAUCLOUD_ERROR_PREFIX))) {
        const code = stableErrorCode(Number(error.status) || 0, { error: error.code });
        const brokerError = new AppServerError(error.message || 'Raucloud could not complete the request', {
          code,
          retryable: error.status === 429
            || /(?:TIMEOUT|UNAVAILABLE|PROVISION|ALLOCATION|RETRY)/.test(code),
          cause: error,
        });
        if (Number.isInteger(error.status)) brokerError.status = error.status;
