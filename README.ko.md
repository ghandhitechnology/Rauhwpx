...
<p align="center">
  지금 열린 문서를 AI 에이전트가 직접 편집하는 HWP/HWPX 편집기.<br />
  Rust → WebAssembly 엔진, 데스크톱 앱과 웹 편집기, 로컬 문서 처리.
</p>

<p align="center">
  Rauhwpx는 Edward Kim의 <a href="https://github.com/edwardkim/rhwp">edwardkim/rhwp</a>에서 시작한 포크입니다.
</p>
...
한국 사무 업무에서는 HWP와 HWPX 문서를 직접 만들고 고쳐야 합니다. Rauhwpx는 파싱, 레이아웃, 렌더링, 편집을 Rust로 작성하고 WebAssembly로 컴파일한 HWP/HWPX 편집기입니다. MCP를 통해 AI 에이전트를 지금 열린 문서에 연결합니다.

에이전트는 문서 구조, 텍스트 범위, 표, 필드, 렌더링된 페이지를 읽고 눈앞의 문서를 편집합니다. 변경 내용은 바로 확인하고 되돌릴 수 있습니다. 파싱과 편집은 브라우저의 WASM 엔진에서 실행되고, 로컬 에이전트 허브는 문서 로직 없이 Studio와 에이전트 CLI를 연결합니다.

## 제품 원칙

- **문서 우선.** 문서가 작업 공간의 중심이고 에이전트는 문서 작업을 돕습니다.
- **사용자가 작성자.** 계획은 승인을 거치고, 변경은 눈에 보이며, 성공한 작업은 한 번에 되돌릴 수 있습니다. 실패한 편집 묶음은 이전 스냅샷으로 복원됩니다.
- **실제 HWP 업무.** 텍스트 추출에 그치지 않고 표, 필드, 페이지 나누기, 도형, 수식과 저장 형식까지 다룹니다.
- **로컬 우선.** 문서 해석과 편집은 사용자 기기에서 처리합니다. 연결한 모델 제공자와 선택 기능은 네트워크를 사용할 수 있습니다.
